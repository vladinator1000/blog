// Generated on 2025-10-23T23:30:06.344Z
...
// Using the mouse:
// mouse.pos - current mouse position (vec2f, 0.0 to 1.0)
// mouse.start - mouse down position (vec2f, 0.0 to 1.0)
...
// Hold alt to test mouse events in the home screen.
...
// Your uniform and storage buffers here
...
export class DefaultRunner {
	...
	async init() {
		...
		this.bilinear = this.device.createSampler({
			magFilter: "linear",
			minFilter: "linear",
			addressModeU: "clamp-to-edge",
			addressModeV: "clamp-to-edge",
		});
		this.trilinear = this.device.createSampler({
			magFilter: "linear",
			minFilter: "linear",
			mipmapFilter: "linear",
			addressModeU: "clamp-to-edge",
		...
		this.nearestRepeat = this.device.createSampler({
			magFilter: "nearest",
			minFilter: "nearest",
			addressModeU: "repeat",
			addressModeV: "repeat",
		});
		this.bilinearRepeat = this.device.createSampler({
			magFilter: "linear",
			minFilter: "linear",
			addressModeU: "repeat",
			addressModeV: "repeat",
		});
		this.trilinearRepeat = this.device.createSampler({
			magFilter: "linear",
			minFilter: "linear",
			mipmapFilter: "linear",
			addressModeU: "repeat",
		...
		this.computeBindGroup = this.device.createBindGroup({
			...
			entries: [
				...
				...META.uniforms.map((uniform) => ({
					binding: uniform.binding,
					resource: { buffer: this.uniformBuffers.get(uniform.name) },
				})),
				...META.storage.map((storage) => ({
					binding: storage.binding,
					resource: { buffer: this.storageBuffers.get(storage.name) },
				})),
			],
		});

		const renderingModule = this.device.createShaderModule({
			code: RENDER_SHADER,
		});
	...
	recreateTextures() {
		...
		if (width <= 0 || height <= 0) {
			console.warn(
				"Invalid canvas dimensions for texture creation:",
				this.size,
			);
			return;
		}
	...
	createMouseData() {
		...
		// start: vec2f
		view.setFloat32(8, this.mouseState.start.x / width, true);
		view.setFloat32(12, this.mouseState.start.y / height, true);
	...
	packUniformValue(u, v, allParams) {
		...
		this.writeTypeFromFlatList(
			dataView,
			0,
			u.type || { name: "f32", size: 4 },
			source,
			0,
		);
		return arrayBuffer;
	...
	collectFlatValuesForStruct(uInfo, params) {
		...
		const flatten = (typeInfo, name, path) => {
			...
			if (
				typeInfo.isStruct &&
				Array.isArray(typeInfo.members) &&
			...
			if (typeInfo.isArray) {
				...
				const value =
					nested !== undefined
						? nested
						: (params[namespaceKey] ?? params[name]);
			...
			const nested = this.getPathValue(params, pathSegments);
			const value =
				nested !== undefined ? nested : (params[namespaceKey] ?? params[name]);
	...
	writeTypeFromFlatList(view, baseOffset, info, srcArr, index) {
		if (info && info.isStruct && Array.isArray(info.members)) {
			for (const member of info.members) {
				index = this.writeTypeFromFlatList(
					view,
		...
		if (name.startsWith("mat")) {
			...
			const rows =
				(dims && dims.rows) ||
				Math.max(1, Math.floor(((info && info.size) || 4) / 4));
	...
	setByFormat(view, byteOffset, formatName, value) {
		...
		if (formatName === "bool") {
			view.setUint32(byteOffset, value ? 1 : 0, true);
			return;
		}
...
export function registerRunner(shaderId, runner, elementId) {
